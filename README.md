...
- It shows **Verified official build** on a match, and **Unrecognized build** only when the publisher *did* release this version and the bytes differ. A version with no published asset yet — the ordinary case for a build newer than the listing — reads as **No published hash yet**, not as a fault.
