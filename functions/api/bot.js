...
var COMMAND_PREFIX = "!";
var COOLDOWN_MS = 2e3;
var MAX_STORED_MESSAGES = 500;
var MAX_CHANNELS = 1e3;
async function onRequest(context) {
  ...
  const state = {
    ...
    messagesProcessed: 0,
    commandsHandled: 0,
    channelMessages: /* @__PURE__ */ new Map(),
    // geohash -> [{ nym, content, created_at, pubkey }]
    nymActivity: /* @__PURE__ */ new Map(),
    // nym -> { lastSeen, channel, lastMessage }
    channelActivity: /* @__PURE__ */ new Map(),
    // geohash -> { count, lastActive }
    commandCooldowns: /* @__PURE__ */ new Map(),
    // geohash -> lastCommandTimestamp
  ...
  server.addEventListener("message", (event) => {
    try {
      ...
      if (Array.isArray(msg) && msg[0] === "BOT:STATUS_REQ") {
        sendStatus({
          uptime: Math.floor((Date.now() - state.startedAt) / 1e3),
          messagesProcessed: state.messagesProcessed,
          commandsHandled: state.commandsHandled,
          connectedRelays: [...state.upstreams.entries()].filter(([, ws]) => ws.readyState === WebSocket.OPEN).length,
...
function connectToRelay(relayUrl, state, server, sendStatus, context) {
  ...
  try {
    ...
    ws.addEventListener("open", () => {
      ...
      const since = Math.floor(Date.now() / 1e3) - 3600;
      const subId = "bot-" + Math.random().toString(36).slice(2, 8);
      ws.send(JSON.stringify([
        "REQ",
        subId,
        { kinds: [2e4], since, limit: 200 }
      ]));
    });
    ...
    ws.addEventListener("close", () => {
      ...
      state.upstreams.delete(relayUrl);
      if (state.serverOpen) {
        setTimeout(() => connectToRelay(relayUrl, state, server, sendStatus, context), 5e3);
      }
    });
...
function handleRelayMessage(raw, relayUrl, state, sendStatus, context) {
  ...
  if (msg[0] === "EVENT" && msg.length >= 3) {
    const event = msg[2];
    if (!event || event.kind !== 2e4) return;
    if (state.seenEvents.has(event.id)) return;
    state.seenEvents.add(event.id);
    if (state.seenEvents.size > 1e4) {
      const arr = [...state.seenEvents];
      state.seenEvents = new Set(arr.slice(-8e3));
    }
...
function trackMessage(state, nym, geohash, content, created_at, pubkey) {
  if (!state.channelMessages.has(geohash)) {
    if (state.channelMessages.size >= MAX_CHANNELS) {
      ...
      for (const [ch, msgs2] of state.channelMessages) {
        if (msgs2.length > 0 && msgs2[msgs2.length - 1].created_at < oldestTime) {
          oldestTime = msgs2[msgs2.length - 1].created_at;
          oldestChannel = ch;
        }
      }
...
function handleTop(state) {
  ...
  const sorted = [...state.channelActivity.entries()].sort((a, b) => b[1].count - a[1].count).slice(0, 10);
...
function handleWho(state) {
  ...
  const activeNyms = [...state.nymActivity.values()].filter((info) => info.lastSeen > oneHourAgo).sort((a, b) => b.lastSeen - a.lastSeen);
...
function handleStats(state) {
  const uptimeSeconds = Math.floor((Date.now() - state.startedAt) / 1e3);
  const hours = Math.floor(uptimeSeconds / 3600);
  const minutes = Math.floor(uptimeSeconds % 3600 / 60);
  const seconds = uptimeSeconds % 60;
  const connectedRelays = [...state.upstreams.entries()].filter(([, ws]) => ws.readyState === WebSocket.OPEN).length;
  return [
    "nymbot stats:",
...
function handleRoll(args) {
  ...
  if (args.trim()) {
    ...
    if (match) {
      ...
      const num2 = parseInt(args.trim());
      if (!isNaN(num2) && num2 > 0) {
        sides = Math.min(num2, 100);
      }
    }
  }
...
function timeSince(unixTimestamp) {
  const seconds = Math.floor(Date.now() / 1e3) - unixTimestamp;
