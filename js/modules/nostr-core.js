...
Object.assign(NYM.prototype, {
    ...
    async saveToNostrProfile() {
        ...
        try {
            ...
            if (signedEvent) {
                ...
                // Mirror to R2 only when the user has real profile data — autogenerated
    ...
    // Whether the user has entered real profile data worth mirroring to R2.
    ...
    async _saveProfileToR2(signedEvent) {
        ...
        try {
            ...
            // Cache our own freshly saved profile so reads skip R2 until it changes.
            this._cacheR2Profile(this.pubkey, signedEvent);
        } catch (_) { }
    },

    // Remember a profile already obtained for a pubkey so repeat lookups skip R2.
    // A newer kind 0 (relay or R2) refreshes the entry; entries expire after the TTL.
    _cacheR2Profile(pubkey, event) {
        if (!pubkey) return;
        if (!this._r2ProfileCache) this._r2ProfileCache = new Map();
        this._r2ProfileCache.set(pubkey, { event: event || null, at: Date.now() });
    },
    ...
    // pubkeys served from R2 (including cache hits) so callers can fall back to
    // relays for the rest.
    async _fetchProfilesFromR2(pubkeys) {
        ...
        if (!this._r2ProfileCache) this._r2ProfileCache = new Map();
        const ttl = this.R2_PROFILE_CACHE_TTL || (5 * 60 * 1000);
        const now = Date.now();
        const toFetch = [];
        for (const pk of pubkeys) {
            if (!/^[0-9a-f]{64}$/.test(pk)) continue;
            const cached = this._r2ProfileCache.get(pk);
            if (cached && (now - cached.at) < ttl) {
                // Already have this profile cached — no repeat R2 lookup needed.
                found.add(pk);
                continue;
            }
            toFetch.push(pk);
    ...
    async handleEvent(event) {
        ...
        if (event.kind === 20000 || event.kind === 23333) {
            ...
            try {
                ...
                // Keep the R2 read cache fresh so a newer profile refreshes the
                // entry instead of forcing another bucket lookup.
                this._cacheR2Profile(pubkey, event);
                ...
                if (pubkey === this.pubkey) {
                    ...
                    // Mirror the authoritative signed profile to R2 so the R2-first
            ...
            if (type === 'EVENT' && data[0] === subId) {
                ...
                if (event && event.kind === 0 && resolvers.has(event.pubkey)) {
                    ...
                    try {
                        ...
                        // Keep the R2 read cache fresh so repeat lookups skip the bucket.
                        this._cacheR2Profile(event.pubkey, event);

                        // Mirror our own authoritative profile to R2 so edits made in
                        // other Nostr clients propagate into R2's cached copy.
