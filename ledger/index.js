// nym-ledger Worker — the standalone host for the NymLedger Durable Object.
//
// GENERATED by scripts/build-ledger.mjs from functions/api/_ledger.js and the
// _d1.js helpers it imports. Do not edit this file: edit those and re-run
// `npm run build:ledger`.
//
// The Pages Functions bind NYM_LEDGER to the class exported here, so an op that
// exists only in functions/api/_ledger.js answers "unknown op" in production —
// which is what happened to the turn, resume and progress ops, silently,
// because turnAcquire degrades rather than failing.

// --- from functions/api/_d1.js ---------------------------------------------
...
// The worker itself serves nothing: it exists to host the Durable Object class
// above, which the Pages Functions reach through their NYM_LEDGER binding.
const worker = {
  async fetch() {
...
export { NymLedger, worker as default };
