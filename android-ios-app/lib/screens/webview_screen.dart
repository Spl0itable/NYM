  ...
  (function() {
    ...
    if (isMobile && (window.FlutterImagePicker || window.FlutterFilePicker)) {
      ...
      const triggerFlutterPicker = (input, source) => {
        ...
        setTimeout(() => { window._nymPickerTriggered = false; }, 500);

        const hasCapture = input.hasAttribute('capture');
        const imageOnly = isImageOnlyInput(input);
        const inputType = getInputType(input);

        console.log('[NYM Bridge] Triggering Flutter picker from:', source, 'input:', input.id, 'type:', inputType, 'imageOnly:', imageOnly, 'hasCapture:', hasCapture);
